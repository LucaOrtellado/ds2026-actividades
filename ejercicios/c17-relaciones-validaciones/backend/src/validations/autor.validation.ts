...
export const autorCreateSchema = z.object({
  nombre: z.string().trim().min(1, "El nombre del autor es obligatorio").max(100),
  nacionalidad: z.string().trim().min(1, "La nacionalidad del autor es obligatoria").max(100),
});
